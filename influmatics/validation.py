...
class InputValidation:
    path: Path
    records: list[SeqRecord] = field(repr=False)
    errors: tuple[str, ...]
    warnings: tuple[str, ...]
...
def validate_sequence_input(path: str | Path, allow_duplicate_ids: bool = False) -> InputValidation:
    ...
    if input_path.stat().st_size == 0:
        return InputValidation(input_path, records, (f"Input file is empty: {input_path}",), ())
    ...
    try:
        ...
        return InputValidation(input_path, records, tuple(errors), tuple(warnings))
    ...
    duplicate_ids = sorted(
        {seq_id for seq_id in normalized_ids if normalized_ids.count(seq_id) > 1}
    )
