...
p <- p +
  ...
  theme(
    ...
    legend.position = "none", # <-- "none"을 "right"로 변경하여 범례 표시
...
## =============================================================================
## 9) [수동+자동 하이브리드] 클레이드 바 위치 조정
## =============================================================================

# 1. 클레이드 목록
target_clades <- unique(as.character(meta2$clade_key))
target_clades <- setdiff(target_clades, c("Pre-clade strains", NA))
target_clades <- sort(target_clades) # 정렬 (선택사항)

# -----------------------------------------------------------------------------
# 🔧 [수동 조정 구역] 유독 겹치는 클레이드 이름과 위치를 여기에 적으세요!
#    형식: "클레이드이름" = 오프셋값
#    (값이 클수록 오른쪽으로 멀어집니다)
# -----------------------------------------------------------------------------
manual_offsets <- c(
  "unassigned"   = -30   # 예: 이 클레이드는 1.5만큼 멀리 보내라
)
manual_text_gaps <- c(
  "unassigned"   = -20   # 예: 막대랑 글자를 확 띄우기 (글자가 길어서 겹칠 때)
)
# 2. 기본 설정 (자동 배치용)
base_offset <- -20    # 기본 시작 위치
step_size   <- 5     # 지그재그 간격

# 3. 반복문 실행
for (i in seq_along(target_clades)) {
  
  cl <- target_clades[i]
  
  tips_in_clade <- meta2 %>% filter(clade_key == cl) %>% pull(label)
  valid_tips <- intersect(tips_in_clade, tree_clean$tip.label)
  
  if (length(valid_tips) > 1) {
    mrca_node <- ape::getMRCA(tree_clean, valid_tips)
    
    bar_col <- cols_named[cl] 
    if(is.na(bar_col)) bar_col <- "black"
    
    # 🔥 [위치 결정 로직] 
    if (cl %in% names(manual_offsets)) {
      # 1) 수동 리스트에 이름이 있으면 -> 그 값을 쓴다!
      final_offset <- manual_offsets[cl]
    } else {
      # 2) 없으면 -> 자동(지그재그) 계산을 한다!
      #    홀수/짝수 번갈아 배치 (i %% 2)
      final_offset <- base_offset + (i %%30) * step_size
    }
...
# 오른쪽 여백 확보
p <- p + theme(plot.margin = margin(t = 5, r = 50, b = 8, l = 5, unit = "mm"))
print(p)